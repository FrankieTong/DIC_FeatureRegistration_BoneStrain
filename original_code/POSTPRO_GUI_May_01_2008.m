...
date_time_pstprs_folder = datestr(date_time_pstprs_run, ' yyyy-mm-dd, HH''MM''SS');

% Define the path where new outputs will be saved
PostProcess_folder_path = sprintf('%s\\Post-Process Outputs for %s', cd, date_time_pstprs_folder);
...
if isequal(Total_or_Increm, 'Total') == true
    s_tmp = sprintf('Total Data_');
else
    s_tmp = sprintf('Incremental Data_');
end

[tmp1, tmp2, tmp3] = mkdir(RD.PostProcess_folder_path, 'Processed Data');
Output_file_name = strcat(RD.PostProcess_folder_path, '\Processed Data\', s_tmp, RD.date_time_short, '.txt');
