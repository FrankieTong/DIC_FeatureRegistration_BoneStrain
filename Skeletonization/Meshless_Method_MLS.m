function [displacement_out, strain_out] = Meshless_Method_MLS(varargin)
...
elseif nargin == 7
    ...
    %Add input matched points and load script file
    fixed_points = varargin{1};
    moving_points = varargin{2};
    
    %Decide which variables are adjustable
    ...
    neighbouring_points_min = varargin{4}; %input

    use_calculated_displacement = varargin{5}; %input
    ...
    display_figures = varargin{6}; %Make this variable work for disabling figures
    
    image_setup = varargin{7};
...
elseif nargin == 8
    ...
    nodal_points_average_spacing = varargin{6};
    ...
    display_figures = varargin{7}; %Make this variable work for disabling figures
    
    image_setup = varargin{8};


    
else
    display 'Inncorrrect number of variables.'
    displacement_out = [NaN];
    strain_out = [NaN];
    return
end
...
if domain_influence_variable == true
	...
	if beta > 0
        ...
        if exist('nodal_points_average_spacing','var')
...
displacement_out = displacement_reshape;
strain_out = strain_reshape;
