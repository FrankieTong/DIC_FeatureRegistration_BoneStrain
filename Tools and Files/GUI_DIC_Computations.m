...
for counter = 1:total_num_subsets
    ...
    switch optim_method 
        ...
        X = Xp + q_good(1) + I;
        Y = Yp + q_good(2) + J;
        g_test = def_interp(Yp+j+q_good(1), Xp+i+q_good(2));
        f_g_sq_test = (f_test-g_test).^2;
        
        % The q_k must be different in this version
        % it needs the first order terms before, and the zeroth order terms
        % after.
    ...
    % If the current correlation quality is 5 times greater than the average quality so far, 
    ...
    if split_subset == false    % If it's a good correlation, update the tolerances
        ...
        f_g_tol   = (f_g_tol*(num_good_corr) + 5*max(f_g_sq_test))/(num_good_corr+1);  % Since this is a good correlation, update the tol of (f-g)^2
        ...
        % Define a logical matrix where 1 means the value is bad, based on a tolerance f_g_tol
        Bad_Values(:,:,num_subsets_split) = reshape(f_g_sq_test > f_g_tol, subset_size, subset_size);
        
        % For the first point, save the current q_good as initial guess
        if first_split == true
            q_0_jump = q_good;
            first_split = false;
        end
...
% guess for the displacements of the first point. Since subset splitting
% makes two parts of the subset deform differently, we need to have two
% sets of deformation parameters

% We saved the last good result before the first subset split was reached.
% This will form the first part of the initial guess (u1 and v1)
q_k_1 = q_0_jump(1:2);

% Now, the other part of the initial guess (u2 and v2) must be obtained by
% performing a coarse search. However, to avoid any bias from the
% points that deform by u1, v1, the subset will be multiplied by
% "Bad_Values" to zero out the points that likely deform by u1, v1.
BV = Bad_Values(:,:,1);
...
% For now... assume that the correct value is within 10 pixels.
range = 10;
u_check = (round(q_k(1)) - range):(round(q_k(1)) + range);
v_check = (round(q_k(2)) - range):(round(q_k(2)) + range);

% Define the intensities of the reference subset
subref = ref_image(Ysplit(1)-floor(subset_size/2):Ysplit(1)+floor(subset_size/2), ...
                   Xsplit(1)-floor(subset_size/2):Xsplit(1)+floor(subset_size/2));
...
for iter1 = 1:numel(u_check);
    for iter2 = 1:numel(v_check);
        subdef = def_image( (Ysplit(1)-floor(subset_size/2)+v_check(iter2)):(Ysplit(1)+floor(subset_size/2)+v_check(iter2)), ...
                            (Xsplit(1)-floor(subset_size/2)+u_check(iter1)):(Xsplit(1)+floor(subset_size/2)+u_check(iter1)) );
        subdef = subdef.*BV;
...
q_k_2(1) = u_check(OFFSET2);
q_k_2(2) = v_check(OFFSET1);
clear u_check v_check iter1 iter2 subref subdef sum_diff_sq TMP1 TMP2 OFFSET1 OFFSET2;

% Initialize the new q_k variable
q_k = [q_k_1(1); q_k_1(2); q_k_2(1); q_k_2(2)];
...
for counter = 1:num_subsets_split
    ...
    X_defcoord_1       = -floor(subset_size/2)+Xp+floor(q_k(1))-interp_buffer:floor(subset_size/2)+Xp+floor(q_k(1))+interp_buffer;
    Y_defcoord_1       = -floor(subset_size/2)+Yp+floor(q_k(2))-interp_buffer:floor(subset_size/2)+Yp+floor(q_k(2))+interp_buffer;
    X_defcoord_2       = -floor(subset_size/2)+Xp+floor(q_k(3))-interp_buffer:floor(subset_size/2)+Xp+floor(q_k(3))+interp_buffer;
    Y_defcoord_2       = -floor(subset_size/2)+Yp+floor(q_k(4))-interp_buffer:floor(subset_size/2)+Yp+floor(q_k(4))+interp_buffer;

    % Fit the interpolating spline: g(x,y) around the two points of interest
    def_interp_1   = spapi( {spline_order,spline_order}, {Y_defcoord_1, X_defcoord_1}, def_image(Y_defcoord_1, X_defcoord_1) );
    def_interp_2   = spapi( {spline_order,spline_order}, {Y_defcoord_2, X_defcoord_2}, def_image(Y_defcoord_2, X_defcoord_2) );

    % Find the partial derivitives of the spline: dg/dx and dg/dy
    def_interp_x_1 = fnder(def_interp_1, [0,1]);
    def_interp_x_2 = fnder(def_interp_2, [0,1]);
    def_interp_y_1 = fnder(def_interp_1, [1,0]);
    def_interp_y_2 = fnder(def_interp_2, [1,0]);
    %_________________________________________________________________________ 
    ...
    while a_b_optim_completed == false
        ...
        [a_new, b_new, top, bottom, main] = Optimize_a_b(q_k, Bad_Val );
        [C_last, GRAD_last, HESS] = C_Zeroth_Split(q_k, a_new, b_new, top, bottom);   % q_k was the result from last point or the user's guess
        optim_completed = false;
        ...
        while optim_completed == false
            ...
            q_k = q_k + delta_q;                                        % q_k+1 = q_k + delta_q
            [C, GRAD, HESS] = C_Zeroth_Split(q_k, a_new, b_new, top, bottom);  % Compute new values
        ...
        % If the values of a and b barely changed, stop the looping completly, otherwise, keep going
        if all( abs([a_new - a ; b_new - b]) < TOL(1) ) && C <= 5*mean_C
            ...
            f_test = ref_image(Yp+j, Xp+i);
            g_test = fnval(def_interp_1, {Yp+j+q_k(2), Xp+i+q_k(1)});
            f_g_sq_test = (f_test-g_test).^2;
            Bad_Val = f_g_sq_test > f_g_tol;
    ...
    if main == 1
        ...
        DEFORMATION_PARAMETERS(jj,ii,9) = a_new;        % line parameter: slope a
        DEFORMATION_PARAMETERS(jj,ii,10) = b_new;       % line parameter: y-intercept b
        DEFORMATION_PARAMETERS(jj,ii,11) = 1-C;         % correlation quality
    else
        % Store the current displacements (the main displacements are u2, v2)
        DEFORMATION_PARAMETERS(jj,ii,1) = q_k(3);       % main displacement u
        DEFORMATION_PARAMETERS(jj,ii,2) = q_k(4);       % main displacement v
        DEFORMATION_PARAMETERS(jj,ii,3) = NaN;          % 1st order def. du1/dx      
        DEFORMATION_PARAMETERS(jj,ii,4) = NaN;          % 1st order def. dv1/dy 
        DEFORMATION_PARAMETERS(jj,ii,5) = NaN;          % 1st order def. du1/dy 
        DEFORMATION_PARAMETERS(jj,ii,6) = NaN;          % 1st order def. dv1/dx 
        DEFORMATION_PARAMETERS(jj,ii,7) = q_k(1);       % secondary displacement u
        DEFORMATION_PARAMETERS(jj,ii,8) = q_k(2);       % secondary displacement v
        DEFORMATION_PARAMETERS(jj,ii,9) = a_new;        % line parameter: slope a
        DEFORMATION_PARAMETERS(jj,ii,10) = b_new;       % line parameter: y-intercept b
        DEFORMATION_PARAMETERS(jj,ii,11) = 1-C;         % correlation quality
    end
