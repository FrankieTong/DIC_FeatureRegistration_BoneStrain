...
%                            Objective Function, 
%                            First Order, for Subset Splitting
...
---------------------------------------------------------------------------
|   DIGITAL IMAGE CORRELATION: CROSS-CORRELATION COEFF. FIRST ORDER SS    |
---------------------------------------------------------------------------
...
This version is used when the deformed image is interpolated
%}

function [C, GRAD, HESS] = C_First_Split_GB( q, Correct_section, good_or_bad )
    ...
    % We want to keep the correct section, reshape it into a vector
    Good_Values = reshape( Correct_section, 1, N);
    
    if isequal(good_or_bad, 'good')
        def_interp = def_interp_G;
        def_interp_x = def_interp_x_G;
        def_interp_y = def_interp_y_G;
    elseif isequal(good_or_bad, 'bad');
        def_interp = def_interp_B;
        def_interp_x = def_interp_x_B;
        def_interp_y = def_interp_y_B;
    end
    
    
%-OBJECTIVE FUNCTION "C"---------------------------------------------------
    % f represents intensities of the discrete points in the ref subset
    f = reshape(ref_image(Yp+j, Xp+i), 1,N).*Good_Values;
    
    % g represents the intensities of the continuous splined def sector
    g = fnval(def_interp, [Y;X]).*Good_Values;
...
%-GRADIENT OF "C"----------------------------------------------------------  
    ...
    dg_dX = fnval(def_interp_x, [Y;X]).*Good_Values;
    dg_dY = fnval(def_interp_y, [Y;X]).*Good_Values;
