function [matched_fixed_points, matched_moving_points, matchedPoints_matchMetric, matchedPoints_maxRatio, matchedPoints_subpixelCorrect] = corrmatch4(fixed_points, moving_points, upsample_scaling, MaxRatio, MatchThreshold, BlockSizeAtOriginalResolution, image_setup)
%{  
...
%}
    ...
    for i = 1:size(fixed_points,1)
        ...
        % Use a modified newton guassian approach to approximate the maximum
        % correlation location.
        
        % End condition is either: 
        %   - Find local maxima in image pixel space
        %   - Local maxima lies outside of subimage (discard)
        %   - Maximum number of possible steps that can be taken inside image 
        %       space is reached (discard)
        
        numberIter = 1;
        maximumIter = numel(fixed_image);
        continueApprox = true;
        offset_displacement = zeros(size(bestMatchForward{2}));
        
        while continueApprox
            
            % Round offset displacement to nearest spacing size interval
            offset_displacement = round(offset_displacement./SpacingSize);
            
            % Calculate new center for moving image
            moving_center = bestMatchForward{2} + offset_displacement;
            
            % Create a 3x3x..x3 (n times) array with [1,1] as the center of the cross correlation field
            cross_corr_corrdinates_min = moving_center - (cross_corr_neighbourhood - 1)/2;
            cross_corr_corrdinates_max = moving_center + (cross_corr_neighbourhood - 1)/2;

            for j = 1:length(moving_center)
            ...
            if ~isequal(first_order_grad, zeros(size(first_order_grad)))
                ...
                % If hessian of matrix is not too small, calculate
                % displacement
                if ~isnan(rcond(hessian_matrix))
                    % Calculate offset increment
                    offset_displacement_inc = -(hessian_matrix\first_order_grad);
                    
                    % Stop if calculated displacement is less than 0.5 of
                    % the spacing size in all directions
                    if isequal(round(offset_displacement_inc'./SpacingSize),zeros(size(offset_displacement)));
                        continueApprox = false;
                    else
                       % Limit changes in displacement by 1 pixel in each direction
                       for offset_index = 1:length(offset_displacement_inc)
                           if offset_displacement_inc(offset_index) > SpacingSize(offset_index)
                               offset_displacement_inc(offset_index) = SpacingSize(offset_index);
                           elseif offset_displacement_inc(offset_index) < -SpacingSize(offset_index)
                               offset_displacement_inc(offset_index) = -SpacingSize(offset_index);
                           end
                       end
                    end
                    
                    offset_displacement = offset_displacement + offset_displacement_inc' ;
                else 
                    offset_displacement = NaN;
                    continueApprox = false;
                end
            ...
            % Stop if correlation value at center is local maximum
            ...
            if all(cross_corr(center_index) >= reshape(cross_corr,1,[]))
                continueApprox = false;
            end
            ...
            % If iteration cound exceeds maximum, quit out of approximation
            % loop
            if (numberIter > maximumIter)
                offset_displacement = NaN;
                continueApprox = false;
            end
