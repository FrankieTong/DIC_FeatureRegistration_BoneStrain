...
function C_total = C_First_Split_GB( q, good_section, bad_section )
    ...
    Good_Section = reshape( good_section, 1, N);
    Bad_Section = reshape( bad_section, 1, N);

    
%-OBJECTIVE FUNCTION "C"---------------------------------------------------
    % f represents intensities of the discrete points in the ref subset
    f = reshape(ref_image(Yp+j, Xp+i), 1,N).*(Good_Section+Bad_Section);
    
    % g represents the intensities of the continuous splined def sector
    gG = fnval(def_interp_G, [YG;XG]).*Good_Section;
    gB = fnval(def_interp_B, [YB;XB]).*Bad_Section;
    g = gG + gB;
