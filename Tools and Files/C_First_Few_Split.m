...
%                            Objective Function, 
%                            First Order, Few Subset Splitting
...
This version is used when there are very few non-zero entries in the Bad_Values
matrix during subset splitting
%}

function [C, GRAD, HESS] = C_First_Few_Split( q, Bad_Values )
    ...
    % We want to keep the good values, so invert the Bad_Values matrix, and
    % reshape it into a vector
    Good_Values = reshape( 1 - Bad_Values, 1, N);
...
%-OBJECTIVE FUNCTION "C"---------------------------------------------------
    ...
    g = fnval(def_interp_G, [Y;X]).*Good_Values;
...
%-GRADIENT OF "C"----------------------------------------------------------  
    ...
    dg_dX = fnval(def_interp_x_G, [Y;X]).*Good_Values;
    dg_dY = fnval(def_interp_y_G, [Y;X]).*Good_Values;
